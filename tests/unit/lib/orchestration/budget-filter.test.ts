...
describe("filterFlightsByBudget", () => {
  it("keeps flights at or below the budget cap", () => {
    const flights = [flight("f1", 1200), flight("f2", 3500)];
    expect(filterFlightsByBudget(flights, 3000).map((f) => f.id)).toEqual(["f1"]);
  });
});

describe("filterHotelsByBudget", () => {
  it("keeps hotels at or below the budget cap", () => {
    const hotels = [hotel("h1", 800), hotel("h2", 3200)];
    expect(filterHotelsByBudget(hotels, 3000).map((h) => h.id)).toEqual(["h1"]);
  });
});

describe("minComboPrice", () => {
  it("returns null when either list is empty", () => {
    expect(minComboPrice([], [hotel("h1", 100)])).toBeNull();
    expect(minComboPrice([flight("f1", 100)], [])).toBeNull();
  });
});
